    ...
    <div class="tab_content">
        ...
        <div btn-default class="search_hotel_btn_wrap">
            <?php $hotelDom = 'hotelType=dom';if (strpos($_SERVER['REQUEST_URI'], $hotelDom) !== false): ?>
                <p class="search s_index btn1_hover fr" data-hotelType="hotel_dom" id="hotel_search" >搜索</p>
            <?php else: ?>
                <p class="search s_index btn1_hover fr" data-hotelType="hotel_inter" id="hotel_search" >搜索</p>
            <?php endif;?>
        </div>
    </div>
</div>
<script>
    (function () {
        function show(index) {
            $('.hotel_list li').removeClass('cur').eq(index).addClass('cur');
            $('[tab-card] .tab_content [input-prompt] input').removeClass('cur').eq(index).addClass('cur');
        }
        $('.hotel_list li').on('click', function (e) {
            show($(this).index());
            console.log($(this).index());
            if($(this).index() == 0){
                $('#hotel_search').attr('data-hoteltype','hotel_inter');
            }else{
                $('#hotel_search').attr('data-hoteltype','hotel_dom');
            }
        });
        show(<?php echo $hotel_card ?>);
    })();
</script>