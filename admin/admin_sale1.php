...
switch ($action) {
	...
	case "list" :
		...
		if ($types == 0) {
			if ($_SESSION["id"] != '1') {
				$sqladd .= " and (t.uid={$_SESSION["id"]} or t.bid={$adminbid})";
			}
		} else {
			if ($_SESSION["id"] != '1') {
				$sqladd .= " and p.userid!={$_SESSION["id"]}";
			}
		}

		if (!empty ($_GET['keyword'])) { //keywords
			$sqladd .= " and (p.keywords like '%" . $_GET['keyword'] . "%'";
			if ($types == 1 && $ctype == 0) {
				$sqladd .= " or p.op_user like '%" . $_GET['keyword'] . "%')";
			} else {
				$sqladd .= " or t.opuser like '%" . $_GET['keyword'] . "%')";
			}
		}
		$orderbysql="order by hid desc";
		$limitsql="limit $start,$perpage";
		$sqlfrom = " from $taba t,$table p where t.id=p.id " . $sqladd;
		$sql = "select p.id,p.title,p.info_id,p.classid,p.classid2,p.go_day,p.go_num,p.keywords,p.price2,p.url,p.city2,p.info,p.op_user,t.hid,t.title name,t.departure,t.price1 price_1,t.price2 price_2,t.userid uid,t.op_type,t.op_user opuser,t.bid $sqlfrom $orderbysql $limitsql";
