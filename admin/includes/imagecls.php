...
class imagecls {
	...
	function save() {
		...
		elseif (!$this->file['is_image']) $this->error_code = -102;
		elseif (!$this->saveFile($this->file['tmp_name'], $this->file['local_target'])) $this->error_code = -103;
	...
	public function thumb($image, $maxWidth = 200, $maxHeight = 50, $gen = 0, $interlace = true, $priex = '', $is_preview = true) {
		...
		if ($info !== false) {
			...
			elseif ($maxWidth == 0) $scale = $maxHeight / $srcHeight;
			...
			//            {
			//                if($is_preview)
			$thumbname = $paths['dirname'] . '/' . $basefilename . '_' . $priex . '.' . strtolower($paths['extension']);
			//                else
	...
	public function water($source, $water, $alpha = 80, $position = "0") {
		...
		if (is_animated_gif($source)) {
			...
			$gif_maker = new GIFEncoder($frames, $framed, 0, 2, 0, 0, 0, "bin" //bin为二进制   url为地址
	);
