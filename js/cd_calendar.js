...
Calender.prototype = {
    ...
    initialize :function (options) {
		...
		this.go_type = options.go_type;
		this.go_time = options.go_time;
		this.tims = this.go_time.indexOf(",")?this.go_time.split(","):[this.go_time];
    ...
    createContainer:function(){
        ...
        document.getElementById("cd_date").appendChild(container);
    },
    // 渲染日期
        ...
        for (i = 1; i <= days; i++) {
            if (year < nowyear) {
                ...
                if (month < nowmonth + 1) {
                    ...
                    if (i > nowdate){
						if(go_type==0){
							pstr = '<a class="live">' + i + '<span class="live_txt">￥'+ price +'</span></a>';
						}else if(go_type==1){
							w = new Date(year, month-1,i).getDay();
							w = w==0?7:w;
							pstr = this.in_Array(w,tims,false)?'<a class="live">' + i + '<span class="live_txt">￥'+ price +'</span></a>':'<a class="live">' + i + '</a>';
						}else if(go_type==2){
							pstr = this.in_Array(i,tims,false)?'<a class="live">' + i + '<span class="live_txt">￥'+ price +'</span></a>':'<a class="live">' + i + '</a>';
						}else{//==3
							pstr = '<a class="live">' + i + '</a>';
						}
						ddHtml.push(pstr);
					}
	...
	in_Array:function(needle,array,bool){//alert('do');
